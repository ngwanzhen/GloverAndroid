...
// BT is on, connecteddevice has hc05 on componentwillmount, componentDidMount-start reading, if data has bang OR button pressed, initiate profile page
...
class test extends Component {
  ...
  read () {
    if (!this.state.connected) {
      alert('you must be connected to device')
    }
    BluetoothSerial.readFromDevice()
  .then((data) => {
    Toast.showShortBottom('Successfuly read')
  })
  .catch((err) => Toast.showShortBottom(err.message))
  }
